...
DROP TABLE IF EXISTS amenity_votes CASCADE;
DROP TABLE IF EXISTS amenities CASCADE;
DROP TABLE IF EXISTS vendor_reviews CASCADE;
DROP TABLE IF EXISTS vendors CASCADE;
DROP TABLE IF EXISTS hawker_reports CASCADE;
DROP TABLE IF EXISTS station_checkins CASCADE;
DROP FUNCTION IF EXISTS public.immutable_date_trunc_hour(timestamptz) CASCADE;
DROP FUNCTION IF EXISTS public.immutable_cast_to_date(timestamptz) CASCADE;

CREATE OR REPLACE FUNCTION public.immutable_date_trunc_hour(t timestamptz)
RETURNS timestamp
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT date_trunc('hour', t AT TIME ZONE 'UTC');
$$;

CREATE OR REPLACE FUNCTION public.immutable_cast_to_date(t timestamptz)
RETURNS date
LANGUAGE sql
IMMUTABLE
AS $$
...
CREATE UNIQUE INDEX IF NOT EXISTS unique_vote_per_hour
  ON amenity_votes(amenity_id, user_id, public.immutable_date_trunc_hour(created_at));
...
CREATE UNIQUE INDEX IF NOT EXISTS unique_review_per_day
  ON vendor_reviews(vendor_id, user_id, public.immutable_cast_to_date(created_at));
...
-- Index for active check-ins queries
CREATE INDEX IF NOT EXISTS idx_checkins_active ON station_checkins(station_code, expires_at);
