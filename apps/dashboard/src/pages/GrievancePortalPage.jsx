...
export default function GrievancePortalPage() {
  ...
  useEffect(() => {
    ...
    const currentToken = localStorage.getItem('railsaathi_token') || localStorage.getItem('token');
    
    if (currentToken !== devToken) {
      localStorage.setItem('railsaathi_token', devToken);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={{
        ...
        {/* Left Column: Form */}
        <div>
          <GrievanceForm currentStep={currentStep} setCurrentStep={setCurrentStep} />
        </div>

        {/* Right Column: Recent Grievances Sidebar */}
        <div>
          <RecentGrievancesSidebar />
        </div>
      </div>
    </div>
  );
