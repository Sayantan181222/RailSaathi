...
async function seedIntents() {
  ...
  const getUser = (index) => SEED_USERS[index % SEED_USERS.length];

  // NDLS->MMCT Monday (25)
  for (let i=0; i<25; i++) {
    intents.push({ user_id: getUser(i), from_station: 'NDLS', to_station: 'MMCT', travel_date: monday, class: 'GEN', crowding_score: 9.5, crowding_label: 'VERY HIGH', is_surge_route: true });
  }

  // NDLS->MMCT Wednesday (8)
  for (let i=0; i<8; i++) {
    intents.push({ user_id: getUser(i), from_station: 'NDLS', to_station: 'MMCT', travel_date: wednesday, class: 'GEN', crowding_score: 5.5, crowding_label: 'MODERATE', is_surge_route: false });
  }

  // ADI->MMCT Friday (20)
  for (let i=0; i<20; i++) {
    intents.push({ user_id: getUser(i), from_station: 'ADI', to_station: 'MMCT', travel_date: friday, class: 'GEN', crowding_score: 8.5, crowding_label: 'HIGH', is_surge_route: true });
  }

  // NDLS->LKO (15)
  for (let i=0; i<15; i++) {
    const d = new Date(); d.setDate(d.getDate() + (i%5));
    intents.push({ user_id: getUser(i), from_station: 'NDLS', to_station: 'LKO', travel_date: d.toISOString().split('T')[0], class: 'GEN', crowding_score: 6.0, crowding_label: 'MODERATE', is_surge_route: false });
  }

  // PNBE->NDLS (12)
  for (let i=0; i<12; i++) {
    const d = new Date(); d.setDate(d.getDate() + (i%3));
    intents.push({ user_id: getUser(i), from_station: 'PNBE', to_station: 'NDLS', travel_date: d.toISOString().split('T')[0], class: 'GEN', crowding_score: 7.0, crowding_label: 'MODERATE', is_surge_route: false });
  }

  // SBC->MAS (10)
  for (let i=0; i<10; i++) {
    intents.push({ user_id: getUser(i), from_station: 'SBC', to_station: 'MAS', travel_date: monday, class: 'GEN', crowding_score: 6.5, crowding_label: 'MODERATE', is_surge_route: false });
  }

  // HWH->PNBE (10)
  for (let i=0; i<10; i++) {
    intents.push({ user_id: getUser(i), from_station: 'HWH', to_station: 'PNBE', travel_date: friday, class: 'GEN', crowding_score: 6.5, crowding_label: 'MODERATE', is_surge_route: false });
  }
  ...
  for (let i=0; i<100; i++) {
    ...
    intents.push({ user_id: getUser(i), from_station: r.from, to_station: r.to, travel_date: d.toISOString().split('T')[0], class: 'GEN', crowding_score: 5.0, crowding_label: 'MODERATE', is_surge_route: false });
  }
...
async function seedCheckins() {
  ...
  const maxNdls = Math.min(SEED_USERS.length, 15);
  // NDLS (15)
  for (let i=0; i<maxNdls; i++) {
    checkins.push({
  ...
  const maxCstm = Math.min(SEED_USERS.length, 3);
  // CSTM (3)
  for (let i=0; i<maxCstm; i++) {
    checkins.push({
      user_id: SEED_USERS[i],
      station_code: 'CSTM',
      lat: 18.940,
...
async function run() {
  try {
    // Fetch users first to populate SEED_USERS
    const { data: dbUsers, error: userErr } = await supabase.from('users').select('id');
    if (userErr || !dbUsers || dbUsers.length === 0) {
      throw new Error('No users found in database. Please run seed.js or seed-demo-user.js first.');
    }
    SEED_USERS = dbUsers.map(u => u.id);
